...
|---|---|---|---|---|---|
| `bin/rvc-keygen/src/exit.rs` 107 | 1 | — | inherit-intentionally | Test re-implements the EIP-7044 Capella cap. Open-ended `>= Capella` is correct. Production definition is `signing_root.rs` 268. | — |
| `crates/beacon/src/client.rs` 405 | 1 | — | inherit-intentionally | Proposer-duties v1/v2 routing (4.5). Open-ended `>= Gloas` keeps later forks on v2 rather than silently falling back to deprecated v1. | 4.5 |
...
| `crates/crypto/src/signing_root.rs` 268 | 1 | — | inherit-intentionally | EIP-7044: voluntary-exit domain stays Capella-capped however many post-Capella forks exist. Open-ended `>=` is the spec. | — |
| `crates/crypto/src/signing_root.rs` 332 | 1 | — | inherit-intentionally | Test mirror of 268 (`legacy_voluntary_exit_root`). Same Capella-cap inherit. | — |
...
| `crates/signer/src/lib.rs` 1145 | 1 | — | inherit-intentionally | Electra aggregate gRPC path: open-ended `>= Gloas` keeps the root RPC rather than the pre-Electra `SignAggregateAndProof` decoder. | 4.20c |
...
| `crates/block-service/src/service/tests/mocks.rs` 583 | 4 | _ | test-only | Test body SSZ picker. Wildcard `_ =>` Deneb body. Mirrors production string dispatch. | — |
| `crates/block-service/src/service/tests/mocks.rs` 597 | 4 | _ | test-only | Blinded-body twin of 583. | — |
| `crates/block-service/src/service/tests/mocks.rs` 690 | 4 | exhaustive | test-only | `matches!` on deneb/electra/fulu for `BlockContents` bytes. Closed string set; `"gloas"` is false. | — |
...
- **4.5 proposer-duties v2.** Class 1 grew by one (`client.rs` 401):
...
- **4.19 fork-aware deadlines.** Class 1 grew by one (`clock.rs` 30):
